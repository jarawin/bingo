...
function Home() {
  ...
  const onJoin = (e) => {
    ...
    const isHasName = name && name.length > 3 && name.length < 20;
    const isHasServerId = serverId && serverId.length == 4;

    if (!isHasName) {
      setError("Please enter your name (4-20 characters)");
      setLoading(false);
      return;
    }
    ...
    if (userId) {
      setUser(userId).then(() => {
        ...
        setLoading(false);
        window.location.href = `/bingo/${serverId}`;
      });
    } else {
      addUser().then((userId) => {
  ...
  const joinLastServer = () => {
    ...
    console.log(lastServerId, lastName, lastUserId);

    if (!lastServerId || !lastName || !lastUserId) {
